...
## 🔴 최우선 발견: 어드민 전 페이지 폼이 작동하지 않던 문제 (H-1, 해결 완료)

### 증상
로그인해도 어드민의 **모든 페이지 내 폼이 완전히 비활성**이었습니다. "Add Day", "저장", 단계 이동 등
어떤 버튼을 눌러도 아무 반응이 없었습니다. 상단바·사이드바(레이아웃 컴포넌트)는 정상 동작해
겉보기에는 멀쩡해 보였고, API는 정상이라 이전 감사에서 드러나지 않았습니다.

### 원인
`src/app/admin/loading.tsx`가 만드는 **라우트 단위 Suspense 경계**입니다.
어드민 페이지는 DB를 조회하느라 서스펜드되고, 본문이 스트리밍(`<template id="B:0">` →
`<div hidden id="S:0">` → `$RC`)으로 전달되는데, **이렇게 스트리밍된 본문이 하이드레이션되지 않았습니다.**

React 폼의 `action`이 SSR 안전장치인
`javascript:throw new Error('React form unexpectedly submitted')` 상태로 남아 있는 것이 결정적 증거였습니다.

### 원인 규명 과정 (배제한 가설)
| 가설 | 검증 | 결과 |
|---|---|---|
| 새로 만든 `AdminShellFrame` 때문 | 우회 후 재빌드 | ❌ 동일하게 실패 |
| 레이아웃의 클라이언트 컴포넌트 간섭 | 레이아웃에서 전부 제거 | ❌ 동일하게 실패 |
| `CalendarLocaleEnforcer`의 DOM 조작 | 제거 후 재빌드 | ❌ 동일하게 실패 |
| `export const dynamic = "force-dynamic"` | 제거 후 재빌드 | ❌ 동일하게 실패 |
| dev 모드 한정 / 빌드 캐시 | `.next` 삭제 + 프로덕션 빌드 | ❌ 프로덕션에서도 재현 |
| Next.js 버전 버그 | 15.5.18 → 15.5.22 업그레이드 | ❌ 동일하게 실패 |
| **`loading.tsx`의 Suspense 경계** | **제거 후 재빌드** | ✅ **모든 폼 정상 동작** |

파트너 포털이 멀쩡했던 이유도 설명됩니다 — 해당 페이지들은 서스펜드되지 않아 스트리밍을 타지 않았습니다.

### 조치
`src/app/admin/loading.tsx`, `src/app/agency/loading.tsx`와 전용 컴포넌트·CSS를 제거했습니다.
로딩 스켈레톤은 사라지지만 **화면 조작 가능 여부가 우선**입니다(측정된 페이지 응답은 0.1~0.3초).
재발 방지를 위해 `loading.tsx`가 다시 생기면 실패하는 테스트를 추가했습니다.

### 검증
- 견적 폼: "Add Day" 클릭 → 일정 1행→2행 실제 증가
- 3단계 스테퍼: 필수값 없으면 이동 차단 → 값 입력 후 1→2→3단계 이동, 입력값 보존, 마지막 단계에서만 제출 버튼
- 공급사 페이지 폼 7개 전부, 예약 페이지 폼 5개 전부 하이드레이션 확인
- 테스트 147/147, typecheck 통과, 프로덕션 빌드 성공
