...
### 남은 항목 (별도 트랙 권장 — 범위·리스크 큼)
- **U-1 CalendarLocaleEnforcer**(HIGH) — 전역 DOM 하이재킹 → `<DateInput>` 컴포넌트/i18n 이전. 하이드레이션 에러 유발, 다수 폼 영향으로 별도 PR 필요.
- **W-1 notifications 미전달**(MEDIUM), **W-2 booking→예약 수동 승격**(MEDIUM), **W-3 확정서 플레이스홀더**(MEDIUM), **U-3 GlobalTextTranslator**(LOW), **P-1 확장성 인덱스/페이지네이션**(정적 감사 트랙).
...
- **이번 세션 후(C-1·C-2·U-2 반영)**: 핵심 업무 플로우 동작 + 파트너 데이터 경계 확립 + 여권 DOB 무결성 확보. **실운영 파일럿 투입이 가능한 수준**으로 올라섬. 단 U-1(날짜입력 UI 안정성)은 실사용 품질을 위해 다음 우선순위로 처리 권장.
