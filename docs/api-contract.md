...
- `PATCH /api/reservations/:id` validates the status change against an allowed-transition map (no arbitrary jumps). Moving to `confirmed` or `cancelled` is high-risk: a `reason` is required and the audit log is written at `high` risk. The status change goes through the `update_reservation_status` RPC so a database trigger records `reservation_status_history` (the route no longer inserts history directly). Invalid transitions return 400.
...
- Invoice creation requires an internal workflow, a non-cancelled reservation with an accepted quote version, and an open settlement. Invoices default to `draft`; issuing (`status: "issued"`) is finance/admin-only and audited at `high` risk. Each line item's `total_amount` must equal `quantity x unit_amount` (400 otherwise). Finalized operation snapshots generate the invoice as a `draft` for finance to review and issue separately (they no longer auto-issue). The invoice stores a version by reservation or `tour_code`, partner-safe line items, bank/payment deadline snapshots, flight details, final itinerary snapshots, collection timing/status, optional deposit data, and audit evidence.
...
- Payment writes require finance/admin role and a client-supplied `idempotencyKey` (400 if missing — no server timestamp fallback). The payment `currency` must equal the invoice currency (400 otherwise), `amount` must be positive, and `confirmed` payments require a `referenceNo`. A reused idempotency key is treated as a replay: the route returns the existing payment (200) instead of overwriting it. The invoice status is refreshed from confirmed payments, overpayment is flagged (`isOverpaid`), and writes are audited at `high` risk.
...
- Notion CSV migration batches are internal-only; validation writes `staging_rows.validation_status`, stores row-level `migration_errors`, approval is allowed only after a validated batch has no errors, and import inserts valid mapped rows into the approved target table with high-risk audit evidence. Import claims the batch atomically (`approved -> imported` conditional update) before inserting so concurrent PATCHes cannot double-import; an insert failure reverts the batch to `failed` with a recorded error.
- Settlement recalculation counts only the latest invoice version per `tour_code` (re-issued invoices are not double-counted) and rejects mixed-currency inputs (422) rather than summing across currencies; a settlement under `review` is not reset to `draft`.
- Supplier message drafts return 409 if a message already exists for the same idempotency key in a non-`draft`/`failed` state (bump the revision number to draft a new one). The delivery worker claims each queued row atomically and refuses to mark messages `sent` in live mode until a real provider is wired.
- Preview/demo responses that bypass authentication (agency inquiry POST, workflow message POST) are only returned when `JHT_DEMO_MODE=on` in a non-production build; otherwise authentication failures return 401/403 so real submissions are never silently dropped.
- Initial admin bootstrap is one-shot via a `system_flags` claim row: once completed it cannot be re-run even if all admin roles are later deleted. The bootstrap and automation/webhook secret checks use timing-safe comparison.
